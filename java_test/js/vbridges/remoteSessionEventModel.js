...
(function($) {
  ...
  function RemoteSessionEventModel() {
    // private
    var PAGESIZE = 20;
    var data = {
      length:0
    ...
    function ensureData(from,to) {
      ...
      var st = jQuery('#serverType').attr('value');
      if (st) {
        clearTimeout(VB.monitoring.timeoutHandle);
        if (st != "ALL") {
          serverType = '&serverType='+st;
        }
      }
      var parms = 'timestamp='+timestamp+startDate+endDate+eventType+serverType+search +
      '&firstResult=' + (fromPage * PAGESIZE) +
      '&maxResults=' + (((toPage - fromPage) * PAGESIZE) + PAGESIZE) +
      '&sortAsc=' + (sortdir>0) +
      '&sortBy=' +sortcol;
      if (VB.monitoring.sessionEventsTotal) {
        parms += '&total='+VB.monitoring.sessionEventsTotal;
      }

      var url = VB.appCtx+'monitoring/listSessionEvents';
    ...
    function onSuccess(resp) {
      ...
      VB.monitoring.sessionEventsTotal = resp.total;

      jQuery.each(resp.events, function(idx) {
        var eventVals = resp.events[idx];
        eventVals['type'] = eventVals['type'].name;
        eventVals['goldMasterOrg'] = eventVals.goldMaster.organization.id;
        eventVals['goldMaster'] = eventVals.goldMaster.title;
        
        if (eventVals.deploymentMode) {
          eventVals['deploymentMode'] = eventVals.deploymentMode.name;
        }
        else {
          eventVals['deploymentMode'] = '';
        }

        if (eventVals.server) {

          if (eventVals.server.type) {
            eventVals['serverType'] = eventVals.server.type.name;
          }
          else {
            eventVals['serverType'] = '';
          }

          if (eventVals.server.hostname) {
            eventVals['server'] = eventVals.server.hostname;
          }
          else {
            eventVals['server'] = eventVals.server.ipAddress;
          }
  ...
  // RemoteSessionEventModel
  $.extend(true, window, {
    VB: {
      monitoring: {
        RemoteSessionEventModel: RemoteSessionEventModel
      }
    }
  });
})(jQuery);
